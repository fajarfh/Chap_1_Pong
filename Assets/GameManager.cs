...
public class GameManager : MonoBehaviour {
	...
	public BallControl bonusBall; // skrip
	public BallControl bombBall; // skrip
	...
	void OnGUI()
	{
		...
		if (GUI.Button(new Rect(Screen.width / 2 - 60, 35, 120, 53), "RESTART"))
		{
			...
			// ...dan restart game.
			ball.SendMessage("RestartGame", 0.2f, SendMessageOptions.RequireReceiver);

			bonusBall.gameObject.SetActive(true);
			bombBall.gameObject.SetActive(true);

			bonusBall.SendMessage("RestartGame", 0.2f, SendMessageOptions.RequireReceiver);
			bombBall.SendMessage("RestartGame", 0.2f, SendMessageOptions.RequireReceiver);
		}
		...
		if (player1.Score == maxScore)
		{
			...
			bonusBall.SendMessage("ResetBall", null, SendMessageOptions.RequireReceiver);
			bombBall.SendMessage("ResetBall", null, SendMessageOptions.RequireReceiver);

			bonusBall.gameObject.SetActive(false);
		...
		if (isDebugWindowShown)
		{
			...
			float ballFriction = ballCollider.sharedMaterial.friction;
		...
		if (GUI.Button(new Rect(Screen.width/2 - 60, Screen.height - 73, 120, 53), "TOGGLE\nDEBUG INFO"))
		{
			...
			trajectory.ballAtCollision.SetActive(trajectory.enabled);
		}
	}
